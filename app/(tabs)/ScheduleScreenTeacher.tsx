...
const ScheduleScreenTeacher = () => {
  ...
  const fetchSchedules = async () => {
    try {
      ...
      const formattedSchedules = response.documents.map((schedule: any) => ({
        id: schedule.$id,
        name: schedule.name,
      ...
      <View style={{ position: 'relative', marginBottom: 18 }}>
        ...
        >
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <TouchableOpacity
              style={{ backgroundColor: '#fff', borderRadius: 18, paddingHorizontal: 16, paddingVertical: 8, shadowColor: '#2563eb', shadowOpacity: 0.10, shadowRadius: 6, shadowOffset: { width: 0, height: 2 }, elevation: 4 }}
              onPress={() => router.push("/schedule/ScheduleCreateScreen")}
            >
              <Text style={{ color: '#2563eb', fontWeight: 'bold', fontSize: 15 }}>+ Create</Text>
            </TouchableOpacity>
          </View>
        ...
        <View style={{ paddingHorizontal: 20, paddingBottom: 24 }}>
          {schedules.length === 0 ? (
            <View style={{ backgroundColor: 'rgba(255,255,255,0.85)', borderRadius: 24, padding: 36, alignItems: 'center', shadowColor: '#2563eb', shadowOpacity: 0.08, shadowRadius: 12, shadowOffset: { width: 0, height: 4 }, elevation: 3 }}>
              ...
              <Text style={{ color: '#94a3b8', textAlign: 'center', marginTop: 8 }}>Create a new schedule to get started</Text>
            </View>
          ) : (
